  ...
  const handleEnhance = async () => {
    ...
    try {
      const response = await fetch("https://api.geminigen.ai/uapi/v1/generate", {
        ...
        body: JSON.stringify({
          type: "text",
          prompt: `Enhance this video generation prompt to be highly detailed and cinematic: "${prompt}"`
        })
      });
  ...
  const handleGenerate = async () => {
    ...
    const pollInterval = setInterval(() => {
      setProgress(p => {
        ...
        if (newProgress < 25) setGenerationStage(stages[0]);
        else if (newProgress < 70) setGenerationStage(stages[1]);
        else if (newProgress < 90) setGenerationStage(stages[2]);
    ...
    try {
      const payload = {
        ...
        style: selectedStyle,
        camera_motion: selectedMotion,
        effects: selectedEffects,
        ...(uploadedImage1 ? { image1: uploadedImage1 } : {}),
        ...(uploadedImage2 ? { image2: uploadedImage2 } : {}),
        ...(uploadedVideo ? { reference_video: uploadedVideo } : {})
      };
      ...
      clearInterval(pollInterval);
      toast.error("Video generation failed. Please check your API key.");
    } finally {
