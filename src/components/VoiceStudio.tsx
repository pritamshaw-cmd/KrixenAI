...
export default function VoiceStudio() {
  ...
  const handleEnhance = async () => {
    ...
    try {
      const response = await fetch("https://api.geminigen.ai/uapi/v1/generate", {
        ...
        body: JSON.stringify({
          type: "text",
          prompt: `Rewrite this text to be more natural and cinematic for voice generation: "${textPrompt}"`
        })
      });
  ...
  const handleGenerate = async () => {
    ...
    try {
      const payload = {
        type: "voice",
        prompt: activeMode === 'tts' ? textPrompt : (activeMode === 'dialogue' ? JSON.stringify(dialogueLines) : "Document reading"),
        voice: selectedVoice.id,
        emotion: selectedEmotion,
        speed: speed,
        format: format,
        mode: activeMode,
        ...(activeMode === 'dialogue' ? { speaker_a: speakerA.id, speaker_b: speakerB.id } : {})
      };
      ...
      clearInterval(pollInterval);
      toast.error("Voice generation failed. Please check your API key.");
    } finally {
